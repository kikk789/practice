...
var personalHistoryGetData = function(){
	...
	$(".user-info-table1, .user-info-table2, .user-info-table3").find("input:not(input[name='file']), select").each(function(){ // 고정 테이블 두곳 루프 돌면서 데이터 획득
		
		var nowLoopData = this; // each 돌때마가 해당 개별 input 태그들의 모든 값이 담김
		var nowLoopId = $(nowLoopData).attr("id")+""; //각 input type의 id값을 가져옴
		var nowLoopValue = $(nowLoopData).val()+""; //값만 저장
		...
		var nowDataPlainText = '"' + nowLoopId + '":"' + nowLoopValue + '"'; // id:value를 키쌍 형식으로 저장(아직까진 스트링 타입)
		var isNew = $("#userIdx").val() == ""?true:false; // isNew==true --> 신규모드  / isNew==false --> 수정모드
		...
		if(nowLoopValue == "" || !isNew && nowLoopId == "userSocialSecunum" || isImg){ // 해당 키에 내용이 없으면 전송 데이터에서 제외(루프는 진행) || 수정모드시 주민등록번호 전송 제외
			...
			userDataObjArr.push(nowDataPlainText); // input의 내용을 push 하여 저장
		}
		
	});
	
	userDataObjPlainText = "{" + userDataObjArr.join(",") + "}"; // 콤마를 기준으로 키쌍 형식으로 저장
...
var ajaxRequestRegisterList = function(){
	...
	var reqData = {
			...
			, "maritalStatusSelect" : $("#maritalStatusSelect").val() // 결혼 여부
...
var getRegisterList = function(){
	...
	for(var i = 0; i < resultLen; i++){ // 얻은 데이터 리스트를 html dom 형태로 변환
		var trText = "<tr>";
			//trText += "<td>" + '<input type="checkbox" id="checkTest" name="xxx" value="yyy">' + "</td>";
			trText += "<td height='25'>" + (isEmpty(resultData[i].USER_IDX) == true?"":resultData[i].USER_IDX) + "</td>";
			trText += "<td height='25'>" + (isEmpty(resultData[i].USER_NAME) == true?"":resultData[i].USER_NAME) + "</td>";
			trText += "<td height='25'>" + (isEmpty(resultData[i].USER_COMP) == true?"":resultData[i].USER_COMP) + "</td>";
			trText += "<td height='25'>" + (isEmpty(resultData[i].USER_DEPT) == true?"":resultData[i].USER_DEPT) + "</td>";
			trText += "<td height='25'>" + (isEmpty(resultData[i].USER_SEX) == true?"":resultData[i].USER_SEX) + "</td>";
			trText += "<td height='25'>" + (isEmpty(resultData[i].CAREER_DATE) == true?"0":resultData[i].CAREER_DATE)+ "년차" + "</td>";
			trText += "<td height='25'>" + (isEmpty(resultData[i].USERREGISTERDATE) == true?"":resultData[i].USERREGISTERDATE) + "</td>";
	...
	// table.find("tbody tr td:not(:first-child)").click(function(){
	var trClick = table.find("tbody tr").click(function(){

		if($("#flagPracList").val()==1){
			var $eveTrObj = $(this);
			//var userIdx = $(this).find("td:nth-child(2)").text(); //열에 체크박스 넣을 때 2로 바꿔야함.
			var userIdx = $(this).find("td:nth-child(1)").text();
			...
			$.ajax({
				...
				success:function(data){
					...
					if(imgName==null){
						...
						$("#thumbNail").attr("src","http://localhost:8070/resources/upload/default.jpg");
					}else{
						$("#thumbNail").attr("src","http://localhost:8070/resources/upload/"+imgName);
					}
				}
			});
...
/*	console.log("--------");
	...
	*/
	...
	for (var key in fixedData) {
		...
		if ("USER_EMAIL"===key){
			let emailEnd = val.substring(val.indexOf('@')); //@ 뒷부분 자르기
			let emailFirst = val.substring(0,val.indexOf('@')); //@ 뒷부분 자르기
			$("#emailDomain").val(emailEnd);
			$("#userEmail").val(emailFirst);
		}
		else{
			$("#"+(eval("convertData."+key)+"")).val(val);

			// console.log('key  ', key);
			// console.log('(eval("convertData."+key)+""):   ', (eval("convertData."+key)+""));
			// console.log("val  ",val);
		}
...
var convertData = {
		...
		, "USER_LEFTSIGHT": 		"userLeftSight"
		, "USER_RIGHTSIGHT": 		"userRightSight"
